...
fix_colours <- function(k, k_min) {
  ...
  if (any(duplicated(component_order)) == T && sum(duplicated(component_order) == 1)) {
...
for (k in k_min:k_max) { ##                          For each K in the data
  refcols <- c(refcols, fix_colours(k,k_min)) ##    Use fix_colours to extract the vector of most correlated column names for each Component in the K.
